...
| [keycloak\start-keycloak-docker.sh](keycloak/start-keycloak-docker.sh) | bash script to create and start a docker container using Keycloak. It also imports the kcidp-app.json file with configuration |
| [keycloak\stop-keycloak-docker.sh](keycloak/stops-keycloak-docker.sh) | bash script to stop and remove the docker container using Keycloak. |
| [keycloak\data\kcidp-app.json](keycloak/data/kcidp-app.json) | Configuration file to import to Keycloak. Contains an app and som claims mapping to make the access token look more like Entra |
...
| [scripts\test-cert-auth.ps1](scripts/test-cert-auth.ps1) | Powershell scripts for calling /api/management using certificate based authentication. Run it from the scripts folder |
| [scripts\test-client-creds-keycloak.ps1](scripts/test-client-creds-keycloak.ps1) | Powershell scripts for calling /api/management using Keycloak access token. Run it from the scripts folder |
| [scripts\test-client-creds-entra.ps1](scripts/test-client-creds-entra.ps1) | Powershell scripts for calling /api/management using Entra ID access token. Run it from the scripts folder |
...
The Keycloak data must match values in [keycloak\data\kcidp-app.json](keycloak\data\kcidp-app.json). 
...
In the below screenshot, the first run is calling /api/management with an Entra ID access token and the app then transitions to a Keycloak access token when callin /api/internal.
The second run is calling /api/management using certificate based authentication and the app then continues to use certificate based authentication when callin /api/internal.

![Script output](media/test-script-output.png)
